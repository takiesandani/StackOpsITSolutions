...
// ════════════════════════════════════════════════════════════════════════════════
// IDENTITY DASHBOARD - CACHED VERSION (Fast endpoint, no API calls)
// ════════════════════════════════════════════════════════════════════════════════
app.get('/api/sunbird/identity-dashboard-cached', authenticateToken, async (req, res) => {
    try {
        ...
        const usersResult = await pool.query(
            'SELECT * FROM identity_users ORDER BY last_updated DESC'
        );
        const dbUsers = usersResult[0] || [];
        ...
        const users = dbUsers.map(u => ({
            ...
            isExternal: u.is_external === 1,
            accountEnabled: u.account_enabled === 1,
            lastSignIn: {
                dateTime: u.last_signin_datetime,
                location: u.last_signin_location,
                device: u.last_signin_device,
                daysSince: u.days_since_signin
            }
        }));
        
        // Fetch risk scores
        const riskResult = await pool.query(
            'SELECT * FROM identity_risk_scores WHERE tenant_id = ? LIMIT 1',
            ['sunbird']
        );
        const riskScores = riskResult[0] && riskResult[0][0] ? riskResult[0][0] : {};
        ...
        const metadata = metadataResult[0] && metadataResult[0][0] ? metadataResult[0][0] : {};
        ...
        return res.json({
            ...
            syncMetadata: {
                lastSync: metadata.last_sync_time,
                nextSync: metadata.next_sync_time,
                syncStatus: metadata.sync_status,
                syncDuration: metadata.sync_duration_seconds
            },
            metrics: {
                totalUsers: metrics.total_users,
                adminUsers: metrics.admin_users,
                mfaEnabledUsers: metrics.mfa_enabled_users,
                mfaPercentage: metrics.mfa_percentage,
                highRiskUsers: metrics.high_risk_users,
                mediumRiskUsers: metrics.medium_risk_users,
                activeUsers24h: metrics.active_users_24h,
                usersWithCompleteProfile: metrics.users_with_complete_profile,
                privilegedUsersWithoutMFA: metrics.privileged_users_without_mfa,
                identityRiskScore: metrics.identity_risk_score
            },
            riskBreakdown: {
                inactivity: {
                    '0-7days': riskScores.inactive_0_7_days,
                    '7-30days': riskScores.inactive_7_30_days,
                    '30-90days': riskScores.inactive_30_90_days,
                    '90+days': riskScores.inactive_90_plus_days
                },
                deviceTrust: {
                    managed: riskScores.device_managed,
                    unmanaged: riskScores.device_unmanaged,
                    unknown: riskScores.device_unknown
                },
                authenticationStrength: {
                    passwordOnly: riskScores.auth_password_only,
                    basicMFA: riskScores.auth_basic_mfa,
                    strongMFA: riskScores.auth_strong_mfa
                }
            },
            users: users
        });
        ...
        console.error('[Identity Dashboard Cached] Error:', error.message);
        return res.status(500).json({ 
            success: false, 
...
// ════════════════════════════════════════════════════════════════════════════════
// IDENTITY DASHBOARD DATABASE SYNC SERVICE (Background Caching)
// Updates identity_metrics, identity_users tables every 1 minute
// ════════════════════════════════════════════════════════════════════════════════

let identitySyncInterval = null;
let isSyncInProgress = false;

function startIdentitySyncService() {
    console.log('[Identity Sync Service] Starting background sync service...');
    
    // Run sync immediately on startup
    syncIdentityDataToDatabase('sunbird').catch(err => 
        console.error('[Identity Sync] Initial sync failed:', err.message)
    );
    
    // Then every 1 minute
    identitySyncInterval = setInterval(() => {
        syncIdentityDataToDatabase('sunbird').catch(err => 
            console.error('[Identity Sync] Scheduled sync failed:', err.message)
        );
    }, 60000); // 60 seconds
    
    console.log('[Identity Sync Service] Background sync scheduled every 1 minute');
}

function stopIdentitySyncService() {
    if (identitySyncInterval) {
        clearInterval(identitySyncInterval);
        identitySyncInterval = null;
        console.log('[Identity Sync Service] Stopped');
    }
}

// Main sync function - fetches from Microsoft Graph and updates DB
async function syncIdentityDataToDatabase(tenantId) {
    if (isSyncInProgress) {
        console.log('[Identity Sync] Sync already in progress, skipping...');
        return;
    }
    ...
    try {
        await updateSyncMetadata(tenantId, 'in_progress', null);
        
        console.log(`[Identity Sync] Starting sync for tenant: ${tenantId}`);
        ...
        console.log(`[Identity Sync] Fetched ${users.length} users, ${roleAssignments.length} roles, ${signIns.length} sign-ins`);
        ...
        // Enrich users with controlled concurrency
        const enrichedUsers = await mapWithConcurrency(users, 8, async (user) => {
            const userRoles = userRoleMap[user.id] || [];
            const hasAdminRole = userRoles.some(r => 
                r.name.toLowerCase().includes('admin') || 
            ...
            return {
                ...
                mail: user.mail,
                job_title: user.jobTitle || 'No Title',
                mobile_phone: user.mobilePhone || 'N/A',
                roles: JSON.stringify(userRoles),
                mfa_enabled: hasMFA ? 1 : 0,
                auth_method_count: authMethods.length,
    ...
    : 0;
        ...
        const usersWithCompleteProfile = enrichedUsers.filter(u => 
            u.job_title !== 'No Title' && u.mobile_phone !== 'N/A'
        ).length;
        const privilegedUsersWithoutMFA = enrichedUsers.filter(u => u.is_admin && !u.mfa_enabled).length;
        
        let identityRiskScore = 0;
        enrichedUsers.forEach(user => {
            if (user.is_admin && !user.mfa_enabled) identityRiskScore += 40;
            if (user.days_since_signin > 999) identityRiskScore += 25;
            if (user.auth_method_count === 0) identityRiskScore += 20;
            if (user.is_external) identityRiskScore += 15;
            if (user.risk_level === 'MEDIUM') identityRiskScore += 10;
            if (user.risk_level === 'HIGH') identityRiskScore += 30;
        });
        identityRiskScore = Math.min(100, Math.round((identityRiskScore / (totalUsers * 40)) * 100));
        
        // Calculate breakdowns
        const inactiveBreakdown = {
            '0_7_days': enrichedUsers.filter(u => u.days_since_signin >= 0 && u.days_since_signin <= 7).length,
            '7_30_days': enrichedUsers.filter(u => u.days_since_signin > 7 && u.days_since_signin <= 30).length,
            '30_90_days': enrichedUsers.filter(u => u.days_since_signin > 30 && u.days_since_signin <= 90).length,
            '90_plus_days': enrichedUsers.filter(u => u.days_since_signin > 90).length
        };
        
        const deviceTrust = { managed: 0, unmanaged: 0, unknown: 0 };
        enrichedUsers.forEach(user => {
            if (user.last_signin_device) {
                const device = user.last_signin_device.toLowerCase();
                if (device.includes('unknown')) {
                    deviceTrust.unknown++;
                } else if (device.includes('managed') || device.includes('iphone') || device.includes('ipad') || device.includes('android')) {
                    deviceTrust.managed++;
                } else {
        ...
        const authStrength = { passwordOnly: 0, basicMFA: 0, strongMFA: 0 };
        enrichedUsers.forEach(user => {
            if (user.auth_method_count === 0 || user.auth_method_count === 1) {
                authStrength.passwordOnly++;
            } else if (user.mfa_enabled && user.auth_method_count >= 2) {
                authStrength.basicMFA++;
            }
        });
        
        // Update database
        await updateIdentityDatabaseCache(
            tenantId,
            enrichedUsers,
            {
                totalUsers, adminUsers, mfaEnabledUsers, mfaPercentage,
                highRiskUsers, mediumRiskUsers, activeUsers24h,
                usersWithCompleteProfile, privilegedUsersWithoutMFA, identityRiskScore
            },
            { inactiveBreakdown, deviceTrust, authStrength }
        );
        
        const syncDuration = Math.round((Date.now() - syncStartTime) / 1000);
        await updateSyncMetadata(tenantId, 'completed', null, totalUsers, syncDuration);
        
        console.log(`[Identity Sync] ✅ Sync completed in ${syncDuration}s. Updated: ${totalUsers} users`);
        
    } catch (error) {
        console.error('[Identity Sync] ❌ Sync failed:', error.message);
        await updateSyncMetadata(tenantId, 'failed', error.message);
    } finally {
        isSyncInProgress = false;
    }
}

async function updateIdentityDatabaseCache(tenantId, enrichedUsers, metrics, breakdowns) {
    if (!pool) {
        console.warn('[Identity Sync] No database pool available');
        return;
    }
    ...
    try {
        ...
        if (enrichedUsers.length > 0) {
            const userInsertValues = enrichedUsers.map((u, idx) => [
                u.id, u.user_principal_name, u.display_name, u.mail, u.job_title,
                u.mobile_phone, u.roles, u.mfa_enabled, u.auth_method_count,
                u.risk_level, u.is_external, u.account_enabled, u.last_signin_datetime,
                u.last_signin_location, u.last_signin_device, u.days_since_signin, u.is_admin
            ]);
            
            await conn.query(
                `INSERT INTO identity_users (
                    id, user_principal_name, display_name, mail, job_title, mobile_phone, roles,
                    mfa_enabled, auth_method_count, risk_level, is_external, account_enabled,
                    last_signin_datetime, last_signin_location, last_signin_device, days_since_signin, is_admin
                ) VALUES ?`,
                [userInsertValues]
            );
        }
        ...
        await conn.query(
            `INSERT INTO identity_metrics (
                tenant_id, total_users, admin_users, mfa_enabled_users, mfa_percentage,
                high_risk_users, medium_risk_users, active_users_24h, users_with_complete_profile,
                privileged_users_without_mfa, identity_risk_score
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                total_users = VALUES(total_users),
                admin_users = VALUES(admin_users),
                mfa_enabled_users = VALUES(mfa_enabled_users),
                mfa_percentage = VALUES(mfa_percentage),
                high_risk_users = VALUES(high_risk_users),
                medium_risk_users = VALUES(medium_risk_users),
                active_users_24h = VALUES(active_users_24h),
                users_with_complete_profile = VALUES(users_with_complete_profile),
                privileged_users_without_mfa = VALUES(privileged_users_without_mfa),
                identity_risk_score = VALUES(identity_risk_score),
                last_updated = NOW()`,
            [
                tenantId, metrics.totalUsers, metrics.adminUsers, metrics.mfaEnabledUsers,
                metrics.mfaPercentage, metrics.highRiskUsers, metrics.mediumRiskUsers,
                metrics.activeUsers24h, metrics.usersWithCompleteProfile,
                metrics.privilegedUsersWithoutMFA, metrics.identityRiskScore
            ]
        );
        ...
        await conn.query(
            `INSERT INTO identity_risk_scores (
                tenant_id, inactive_0_7_days, inactive_7_30_days, inactive_30_90_days,
                inactive_90_plus_days, device_managed, device_unmanaged, device_unknown,
                auth_password_only, auth_basic_mfa, auth_strong_mfa
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                inactive_0_7_days = VALUES(inactive_0_7_days),
                inactive_7_30_days = VALUES(inactive_7_30_days),
                inactive_30_90_days = VALUES(inactive_30_90_days),
                inactive_90_plus_days = VALUES(inactive_90_plus_days),
                device_managed = VALUES(device_managed),
                device_unmanaged = VALUES(device_unmanaged),
                device_unknown = VALUES(device_unknown),
                auth_password_only = VALUES(auth_password_only),
                auth_basic_mfa = VALUES(auth_basic_mfa),
                auth_strong_mfa = VALUES(auth_strong_mfa),
                last_updated = NOW()`,
            [
                tenantId,
                breakdowns.inactiveBreakdown['0_7_days'],
                breakdowns.inactiveBreakdown['7_30_days'],
                breakdowns.inactiveBreakdown['30_90_days'],
                breakdowns.inactiveBreakdown['90_plus_days'],
                breakdowns.deviceTrust.managed,
                breakdowns.deviceTrust.unmanaged,
                breakdowns.deviceTrust.unknown,
                breakdowns.authStrength.passwordOnly,
                breakdowns.authStrength.basicMFA,
                breakdowns.authStrength.strongMFA
            ]
        );
...
async function updateSyncMetadata(tenantId, status, errorMessage, userCount = 0, duration = 0) {
    if (!pool) return;
    
    try {
        const nextSyncTime = new Date(Date.now() + 60000);
        
        await pool.query(
            `INSERT INTO identity_cache_metadata (
                tenant_id, last_sync_time, next_sync_time, sync_status, sync_error_message,
                total_users_synced, sync_duration_seconds
            ) VALUES (?, NOW(), ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                last_sync_time = NOW(),
                ...
                total_users_synced = VALUES(total_users_synced),
                sync_duration_seconds = VALUES(sync_duration_seconds)`,
            [tenantId, nextSyncTime, status, errorMessage || null, userCount, duration]
        );
    } catch (error) {
        console.error('[Sync Metadata] Failed to update:', error.message);
    }
}
...
const server = app.listen(PORT, async () => {
    ...
    // Start Identity Dashboard background sync service
    if (pool) {
        startIdentitySyncService();
    } else {
        console.warn('⚠️  Database not available - Identity Sync Service not started');
    }
});
