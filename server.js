...
// Fetch managed devices from Intune/Microsoft Graph
async function fetchManagedDevices(token) {
  try {
    const response = await fetch(
      'https://graph.microsoft.com/v1.0/deviceManagement/managedDevices?$top=999&$select=id,deviceName,userPrincipalName,operatingSystem,complianceState,isEncrypted,managementAgent,lastSyncDateTime,azureADRegistered,deviceEnrollmentType,osVersion,manufacturer,model,imei',
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    console.error('[Microsoft Graph] Failed to fetch managed devices:', error.message);
    return [];
  }
}

// Fetch device compliance policies
async function fetchDeviceCompliancePolicies(token) {
  try {
    const response = await fetch(
      'https://graph.microsoft.com/v1.0/deviceManagement/deviceCompliancePolicies?$top=999&$select=id,displayName,description,isScheduledActionEnabled,scheduledActionConfigurations',
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    console.error('[Microsoft Graph] Failed to fetch compliance policies:', error.message);
    return [];
  }
}

// Fetch device configurations
async function fetchDeviceConfigurations(token) {
  try {
    const response = await fetch(
      'https://graph.microsoft.com/v1.0/deviceManagement/deviceConfigurations?$top=999&$select=id,displayName,description,lastModifiedDateTime',
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    console.error('[Microsoft Graph] Failed to fetch device configurations:', error.message);
    return [];
  }
}
...
async function fetchSecurityAlerts(token) {
  try {
    const response = await fetch(
      'https://graph.microsoft.com/v1.0/security/alerts?$top=999&$select=id,title,description,severity,status,createdDateTime,eventDateTime',
      {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    if (!response.ok) {
      // Alerts endpoint may not be available, return empty
      return [];
    }
    ...
    console.error('[Microsoft Graph] Failed to fetch security alerts:', error.message);
    return [];
  }
}
...
function calculateDeviceRiskScore(device) {
  ...
  if (device.lastSyncDateTime) {
    const lastSyncDays = Math.floor((Date.now() - new Date(device.lastSyncDateTime).getTime()) / (1000 * 60 * 60 * 24));
    if (lastSyncDays > 7) {
      riskScore += 20;
    }
  }

  // No management agent → +25
  if (!device.managementAgent || device.managementAgent === 'unknown') {
    riskScore += 25;
  }

  // Not Azure AD registered → +15
  if (device.azureADRegistered !== true) {
    riskScore += 15;
  }
...
// Calculate device activity status based on last sync
function getActivityStatus(lastSyncDateTime) {
  if (!lastSyncDateTime) return 'UNKNOWN';
  
  const lastSyncDays = Math.floor((Date.now() - new Date(lastSyncDateTime).getTime()) / (1000 * 60 * 60 * 24));
  
  if (lastSyncDays === 0) return 'ACTIVE_24H';
  if (lastSyncDays <= 7) return 'ACTIVE_7D';
  if (lastSyncDays <= 30) return 'INACTIVE_30D';
  return 'INACTIVE_90D';
}

// Aggregate and process device data for dashboard
function processDevicesDataForDashboard(devices) {
  const now = new Date();
  ...
  const metrics = {
    ...
    nonCompliantDevices: devices.filter(d => d.complianceState !== 'compliant').length,
    encryptedDevices: devices.filter(d => d.isEncrypted === true).length,
    unencryptedDevices: devices.filter(d => d.isEncrypted !== true).length,
    
    // Activity metrics
    activeDevices: devices.filter(d => d.lastSyncDateTime && new Date(d.lastSyncDateTime) > last24h).length,
    staleDevices: devices.filter(d => d.lastSyncDateTime && new Date(d.lastSyncDateTime) <= last7d).length,
    inactiveDevices: devices.filter(d => d.lastSyncDateTime && new Date(d.lastSyncDateTime) <= last30d).length,
    
    // OS distribution
    osDistribution: {
      windows: devices.filter(d => d.operatingSystem?.toLowerCase().includes('windows')).length,
      ios: devices.filter(d => d.operatingSystem?.toLowerCase().includes('ios')).length,
      android: devices.filter(d => d.operatingSystem?.toLowerCase().includes('android')).length,
      macos: devices.filter(d => d.operatingSystem?.toLowerCase().includes('mac')).length,
      unknown: devices.filter(d => !d.operatingSystem || (
        ...
        !d.operatingSystem.toLowerCase().includes('mac')
      )).length
    },
    ...
    complianceBreakdown: {
      ...
      unknown: devices.filter(d => !d.complianceState || (d.complianceState !== 'compliant' && d.complianceState !== 'noncompliant')).length
    },
    
    // Management status
    managementStatus: {
      intuneManaged: devices.filter(d => d.managementAgent && d.managementAgent.toLowerCase().includes('intune')).length,
      azureADRegistered: devices.filter(d => d.azureADRegistered === true).length,
      unmanaged: devices.filter(d => !d.managementAgent || d.managementAgent === 'unknown').length
    }
  };

  // Calculate percentages
  if (metrics.totalDevices > 0) {
    metrics.compliancePercentage = Math.round((metrics.compliantDevices / metrics.totalDevices) * 100);
    metrics.encryptionPercentage = Math.round((metrics.encryptedDevices / metrics.totalDevices) * 100);
    metrics.activePercentage = Math.round((metrics.activeDevices / metrics.totalDevices) * 100);
  }
  ...
  const devicesWithRisk = devices.map(device => ({
    ...
    activityStatus: getActivityStatus(device.lastSyncDateTime)
  }));

  metrics.highRiskDevices = devicesWithRisk
    .filter(d => {
      const riskLevel = getRiskLevel(d.riskScore);
      return riskLevel === 'HIGH' || (d.complianceState !== 'compliant' && d.isEncrypted !== true);
    })
  ...
  // Device health scores (normalized to 100)
  const healthScores = {
    compliance: metrics.totalDevices > 0 ? Math.round((metrics.compliantDevices / metrics.totalDevices) * 100) : 0,
    encryption: metrics.totalDevices > 0 ? Math.round((metrics.encryptedDevices / metrics.totalDevices) * 100) : 0,
    activity: metrics.totalDevices > 0 ? Math.round((metrics.activeDevices / metrics.totalDevices) * 100) : 0,
    management: metrics.totalDevices > 0 ? Math.round((metrics.managementStatus.intuneManaged / metrics.totalDevices) * 100) : 0,
    threat: Math.round(100 - (metrics.highRiskDevices.length / Math.max(metrics.totalDevices, 1) * 100))
  };

  metrics.healthScores = healthScores;
  metrics.overallHealthScore = Math.round(
    (healthScores.compliance * 0.3 + 
     healthScores.encryption * 0.2 + 
     healthScores.activity * 0.2 + 
     healthScores.management * 0.15 + 
     healthScores.threat * 0.15)
  );
 ...
 * Returns: Aggregated device metrics, health scores, risk summary
 */
app.get('/api/devices/summary', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching summary for: ${userEmail}`);

        // Get the tenant for this user
        const tenant = getTenantByEmail(userEmail);
        if (!tenant || tenant.clientId !== 'sunbird') {
            console.warn(`[Devices] Access denied for ${userEmail}`);
            return res.status(403).json({ 
                error: 'Access denied',
                message: 'Device management is only available for authorized clients'
            });
        }
        ...
        const metrics = processDevicesDataForDashboard(devices);

        // Fetch alerts for additional context
        ...
        console.log(`[Devices] Summary generated for ${metrics.totalDevices} devices`);
        ...
        console.error('[Devices] Error fetching summary:', error.message);
        res.status(500).json({ 
 ...
 * Returns: Full list of managed devices with risk scores and activity status
 */
app.get('/api/devices/list', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching device list for: ${userEmail}`);
        ...
        // Enrich with risk scores and activity status
        const enrichedDevices = devices.map(device => ({
            ...
            riskScore: calculateDeviceRiskScore(device),
            riskLevel: getRiskLevel(calculateDeviceRiskScore(device)),
            activityStatus: getActivityStatus(device.lastSyncDateTime)
        }))
        .sort((a, b) => b.riskScore - a.riskScore);

        console.log(`[Devices] Retrieved ${enrichedDevices.length} devices`);
        ...
        console.error('[Devices] Error fetching device list:', error.message);
        res.status(500).json({ 
...
/**
 * Route: GET /api/devices/high-risk
 * Returns: Top 10 high-risk devices
 */
app.get('/api/devices/high-risk', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching high-risk devices for: ${userEmail}`);
        ...
        // Fetch devices and get high-risk list
        const devices = await fetchManagedDevices(token);
        const metrics = processDevicesDataForDashboard(devices);

        console.log(`[Devices] Identified ${metrics.highRiskDevices.length} high-risk devices`);

        res.json({
            success: true,
            tenant: tenant.clientId,
            totalHighRisk: metrics.highRiskDevices.length,
            devices: metrics.highRiskDevices,
            fetchedAt: new Date().toISOString()
        });

    } catch (error) {
        console.error('[Devices] Error fetching high-risk devices:', error.message);
        res.status(500).json({ 
            error: 'Failed to fetch high-risk devices',
            message: error.message
        });
    }
});
...
/**
 * Route: GET /api/devices/distributions
 * Returns: OS distribution, compliance breakdown, management status
 */
app.get('/api/devices/distributions', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching distribution data for: ${userEmail}`);
        ...
        // Fetch devices and calculate distributions
        const devices = await fetchManagedDevices(token);
        const metrics = processDevicesDataForDashboard(devices);
        ...
        res.json({
            success: true,
            tenant: tenant.clientId,
            osDistribution: metrics.osDistribution,
            complianceBreakdown: metrics.complianceBreakdown,
            managementStatus: metrics.managementStatus,
            fetchedAt: new Date().toISOString()
        });

    } catch (error) {
        console.error('[Devices] Error fetching distributions:', error.message);
        res.status(500).json({ 
...
/**
 * Route: GET /api/devices/alerts
 * Returns: Security alerts related to devices
 */
app.get('/api/devices/alerts', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching security alerts for: ${userEmail}`);
        ...
        console.log(`[Devices] Retrieved ${processedAlerts.length} security alerts`);
        ...
        console.error('[Devices] Error fetching alerts:', error.message);
        res.status(500).json({ 
...
/**
 * Route: GET /api/devices/configurations
 * Returns: Device configuration policies
 */
app.get('/api/devices/configurations', authenticateToken, async (req, res) => {
    try {
        const userEmail = req.user.email;
        console.log(`[Devices] Fetching configurations for: ${userEmail}`);
        ...
        const token = await getMicrosoftGraphToken();
        const configs = await fetchDeviceConfigurations(token);
        ...
        console.error('[Devices] Error fetching configurations:', error.message);
        res.status(500).json({ 
            error: 'Failed to fetch device configurations',
            message: error.message
        });
    }
});
