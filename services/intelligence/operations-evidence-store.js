...
const OPERATIONS_EVIDENCE_SCHEMA = [
    `CREATE TABLE IF NOT EXISTS StackCTRLOperationsEvidenceSnapshots (
        ...
        ApiTasks INT NOT NULL DEFAULT 0,
        ManualTasksExcluded INT NOT NULL DEFAULT 0,
        HighPriorityTasks INT NOT NULL DEFAULT 0,
        MediumPriorityTasks INT NOT NULL DEFAULT 0,
        LowPriorityTasks INT NOT NULL DEFAULT 0,
        OperationsHealthScore DECIMAL(6,2) NOT NULL DEFAULT 0,
        ...
        PRIMARY KEY (ID),
        KEY ix_operations_evidence_latest (CompanyID, IsComplete, CollectedAt, ID),
        KEY ix_operations_evidence_status (CollectionStatus, CollectedAt)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`,
    `CREATE TABLE IF NOT EXISTS StackCTRLOperationsEvidence (
        ID BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
        ...
        CreatedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (ID),
        UNIQUE KEY uq_operations_evidence_snapshot_source (SnapshotID, EvidenceKind, SourceID),
        KEY ix_operations_evidence_snapshot (SnapshotID, ID),
        CONSTRAINT fk_operations_evidence_snapshot
            FOREIGN KEY (SnapshotID) REFERENCES StackCTRLOperationsEvidenceSnapshots(ID)
            ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`
];
...
function operationsSourceId(task, index) {
    return String(`${task.area || 'area'}:${task.task || 'task'}`).slice(0, 255) || `task-${index + 1}`;
}

function deriveOperationsEvidence(payload = {}) {
    ...
    const riskEngine = buildRiskEngine({
        sources: [{ sourceKey: 'operations', status: 'available', isExpected: true, metrics, dashboardMetrics: metrics }],
        dataCompleteness: { score: 100 }
    });
    const stackctrlHealthScore = riskEngine.domainHealthScores.operations;
    const stackctrlRiskScore = riskEngine.domainRiskScores.operations;
    const dashboardMetrics = { ...metrics, stackctrlRiskScore, stackctrlHealthScore };
    const evidenceRows = dashboardSource.tasks.map((task, index) => ({
        kind: 'task',
        sourceId: operationsSourceId(task, index),
        title: String(task.task || 'Operational task').slice(0, 500),
        area: task.area || null,
        status: task.priority || null,
        processed: task
    }));
...
function createOperationsEvidenceStore({ pool, logger = console, now = () => new Date() } = {}) {
    ...
    async function persistProcessedEvidence({ companyId, tenantKey = 'sunbird', payload, collectionTrigger = 'scheduled_hourly', sourceEndpoint = 'Microsoft Graph processed by StackCTRL Operations' } = {}) {
        ...
        try {
            ...
            const [snapshotResult] = await connection.query(
                `INSERT INTO StackCTRLOperationsEvidenceSnapshots
            ...
            for (const row of evidence.evidenceRows) {
                await connection.query(
                    `INSERT INTO StackCTRLOperationsEvidence
...
module.exports = { OPERATIONS_EVIDENCE_SCHEMA, createOperationsEvidenceStore, deriveOperationsEvidence };
